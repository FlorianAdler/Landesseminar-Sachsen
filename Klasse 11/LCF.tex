...
\end{wrapfigure}
Wir wollen die Summe $f(x_1)+f(x_2)+\dotsb+f(x_n)$ minimieren, um zu zeigen, dass sie stets $\geqslant c$ ist. Dazu beginnen wir mit einer beliebigen Wahl der Variablen $x_1,x_2,\dotsc,x_n$ mit $x_1+x_2+\dotsb+x_n=b$. Dann verschieben wir die Variablen schrittweise, sodass $f(x_1)+f(x_2)+\dotsb+f(x_n)$ in jedem Verschiebungsschritt kleiner wird. Dabei gilt:
\begin{itemize}\itshape
	\item[$(*)$] Die $x_i\geqslant a$ werden \enquote{zusammengeschoben}, die $x_i<a$ werden \enquote{auseinandergeschoben}.
\end{itemize}
Das bedeutet konkret: Zuerst können wir alle Variablen $x_i\geqslant a$ durch ihren Mittelwert ersetzen (\enquote{zusammenschieben}), denn für $x\geqslant a$ ist $f$ konvex und die Jensensche Ungleichung gilt. Angenommen, es gibt mindestens zwei Variablen $x_j,x_k<a$. Dann können wir $x_j$ und $x_k$ durch $x_j-\delta$ und $x_k+\delta$ für passendes $\delta>0$ ersetzen (\enquote{auseinanderschieben}). Aus der Ungleichung von Karamata folgt nämlich
\begin{equation*}
...
An den folgenden Beispielaufgaben könnt ihr die Karamata-Schiebemethode trainieren.
...
\end{wrapfigure}
Betrachte nämlich $x_1,x_2,\dotsc,x_n$ mit Mittelwert $\overline{x}\coloneqq\frac{x_1+x_2+\dotsb+x_n}{n}$. Wenn die Tangente an den Graphen von $f$ in $\overline{x}$ die Gleichung $y=ax+b$ hat, dann ist also $f(x)\geqslant ax+b$ für alle $x$, mit Gleichheit für $x=\overline{x}$. Nun folgt
\begin{align*}
...
\end{align*}
wie gewünscht. Auf die gleiche Weise lässt sich die gewichtete Jensen-Ungleichung beweisen. Insbesondere folgt:
\begin{itemize}\itshape
	\item[$(*)$] Um die Jensensche Ungleichung für eine differenzierbare Funktion zu verwenden, benötigen wir nicht Konvexität, sondern nur, dass die Tangente an den Funktionsgraphen im Mittelwert der Variablen stets unterhalb des Funktionsgraphen verläuft.
\end{itemize}
...
Versucht nun, die Jensen-Tangentenmethode auf die folgenden Beispielaufgaben anzuwenden.
...
Die folgenden Beispielaufgaben lassen sich ebenfalls durch geeignete Linearisierung lösen.
