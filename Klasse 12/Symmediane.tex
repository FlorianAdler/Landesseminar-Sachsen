...
Wenn in einer Aufgabe Symmediane vorkommen, dann tun sie das meistens auf \enquote{versteckte} Art und Weise: nämlich in Form des folgenden Lemmas. Dieses Lemma zu kennen, und damit Symmediane in eurer Skizze zu erkennen, kann in der Olympiade sehr nützlich sein, weil es eine ungemein praktische Winkelgleichheit liefert.
\begin{satzmitnamen}[Symmedian-Lemma]
	Sei $ABC$ ein Dreieck mit Umkreis $\Omega$ und sei $T$ der Schnittpunkt der Tangenten in $B$ und $C$ an $\Omega$. Dann ist $AT$ der Symmedian durch $A$. Insbesondere gilt die Winkelgleichheit $\winkel BAT=\winkel MAC$, wobei $M$ den Mittelpunkt von $\overline{BC}$ bezeichnet.
\end{satzmitnamen}
\begin{figure}[ht]
	\centering
